	...
	<div class="relative w-full max-w-[440px]">
		...
		<div class="rounded-[14px] border border-border bg-bg-elev px-7 pt-7 pb-6 shadow-card">
			{#if form?.sent}
				...
				<h1 class="mt-4 text-center text-[22px] font-semibold tracking-[-0.012em] text-text">
					{m.auth_forgot_check_email()}
				</h1>
				<p class="mt-1.5 text-center text-[14px] leading-relaxed text-text-3">
					{m.auth_forgot_sent_message({ email: form.email })}
				</p>
				<a
					href="/login"
					class="mt-6 inline-flex h-10 w-full items-center justify-center rounded-[8px] border border-border bg-surface text-[14px] font-medium text-text-2 transition-colors hover:bg-surface-2 hover:text-text"
				>
