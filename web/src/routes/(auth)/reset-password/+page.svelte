<script lang="ts">
	...
	const invalidLink = $derived(!data.token || data.linkError === 'INVALID_TOKEN');
	const minLength = 8;
	...
	$effect(() => {
		if (!invalidLink) passwordInput?.focus();
	});
...
<svelte:head>
	<title>{m.auth_reset_page_title({ brand: brandName() })}</title>
</svelte:head>
