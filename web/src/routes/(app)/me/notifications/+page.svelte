...
<header class="mb-6 flex items-end justify-between gap-4">
	<div>
		...
		<p class="mt-1 text-[12.5px] text-text-3">{m.notif_subtitle()}</p>
	</div>
	...
	{#each groups as group (group.title)}
		<section class="overflow-hidden rounded-2xl border border-border bg-bg-elev">
			<div class="flex items-center justify-between px-5 pt-4 pb-2">
				...
				<div
					class="grid grid-cols-2 gap-x-8 pr-1 text-[10.5px] tracking-[0.08em] text-text-4 uppercase"
				>
				...
				{#each group.items as item (item.key)}
					...
					<div class="flex items-center gap-4 px-5 py-3">
						<div class="min-w-0 flex-1">
							...
							<div class="mt-0.5 text-[11.5px] text-text-3">{item.desc}</div>
						</div>
