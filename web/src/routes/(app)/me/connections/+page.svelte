<script lang="ts">
	...
	// Personal instructions: the draft follows the server value until edited and
...
<section class="mb-6 rounded-2xl border border-border bg-bg-elev p-5">
	...
	{#if data.mcp.enabled}
		...
		<div class="mt-5 border-t border-border pt-5">
			<h3 class="text-[13px] font-medium text-text-2">{m.connections_instructions_title()}</h3>
			<p class="mt-0.5 text-[13px] text-text-3">{m.connections_instructions_hint()}</p>
			<form method="post" action="?/saveInstructions" use:enhance={onSaveInstructions} class="mt-3">
				<textarea
					name="instructions"
					bind:value={instructions}
					rows="5"
					spellcheck="false"
					placeholder={m.connections_instructions_placeholder()}
					class="w-full resize-y rounded-lg border border-border bg-bg px-3 py-2.5 font-mono text-[13px] leading-relaxed text-text outline-none placeholder:text-text-4 focus:border-border-strong"
				></textarea>
					...
					<div class="ml-auto flex items-center gap-2">
						{#if data.instructionsUpdatedAt}
							<span class="text-[12px] text-text-4"
								>{m.mcp_instructions_updated({
									date: fmt.format(data.instructionsUpdatedAt)
								})}</span
							>
						{/if}
						<Button
		...
		<div class="mt-5 border-t border-border pt-5">
			<div class="mb-3 flex items-end gap-3">
				<div class="min-w-0 flex-1">
					<h3 class="text-[13px] font-medium text-text-2">{m.connections_guides_title()}</h3>
					<p class="mt-0.5 text-[13px] text-text-3">
						{m.connections_guides_hint({ max: data.guideLimit })}
					</p>
				</div>
				{#if data.guides.length < data.guideLimit}
					<a
						href="/me/connections/guides/new"
						class="inline-flex h-8 shrink-0 items-center gap-1.5 rounded-lg border border-border bg-surface px-2.5 text-[13px] text-text-2 transition-colors hover:text-text"
					>
			...
			<div class="overflow-hidden rounded-xl border border-border bg-bg">
				{#if data.guides.length === 0}
					<EmptyState
						icon="book"
						title={m.mcp_guides_empty_title()}
						hint={m.mcp_guides_empty_hint()}
					/>
				{:else}
					<div
						class="grid h-9 items-center gap-3 border-b border-border px-4 text-[12px] tracking-[0.08em] text-text-4 uppercase"
					...
					{#each data.guides as g (g.id)}
						...
						>
							<div class="min-w-0">
								<a
									href="/me/connections/guides/{g.id}"
							...
							<div class="truncate font-mono text-[12px] text-text-3">
								{fmt.format(g.updatedAt)}
							</div>
							<div class="flex items-center gap-2">
								<span class="h-2 w-2 rounded-full {g.enabled ? 'bg-emerald-400' : 'bg-text-4'}"
								></span>
							...
							<div class="flex items-center justify-end gap-1">
								{#if g.sourceUrl}
									<form
										method="post"
										action="?/guideRefresh"
										use:enhance={simple(m.mcp_guide_fetched_toast())}
									>
										...
										>
											<Icon
												name="refresh"
												size={14}
												class={busyId === g.id ? 'animate-spin' : ''}
											/>
										</button>
									</form>
								{/if}
								...
								<form
									method="post"
									action="?/guideDelete"
									use:enhance={simple(m.mcp_guide_deleted_toast())}
								>
									<input type="hidden" name="id" value={g.id} />
									<button
										type="button"
