			...
			<div class="overflow-hidden rounded-2xl border border-border bg-bg-elev">
				...
				{#each visible as o (o.id)}
					<a
						...
						class="grid items-center gap-3 border-b border-border/40 px-5 py-2.5 text-[13px] transition-colors last:border-b-0 hover:bg-[var(--row-hover)] {o.archivedAt
					...
					>
						...
						<span class="truncate font-mono text-[12px] text-text-3">{o.slug}</span>
						<span class="text-right font-mono text-[12px] text-text">{o.projectCount}</span>
						<span class="font-mono text-[12px] text-text-3">{fmtDate(o.createdAt)}</span>
