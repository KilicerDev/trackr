...
<svelte:head
	><title>{pageTitle(`${m.settings_tab_devices()} · ${m.admin_settings_title()}`)}</title
	></svelte:head
>
...
<div class="overflow-x-auto rounded-2xl border border-border bg-bg-elev">
	...
	{:else}
		...
		{#each data.devices as d (d.id)}
			<div
				class="grid min-w-[880px] items-center gap-3 border-b border-border/40 px-5 py-2.5 text-[14px] last:border-b-0"
				style:grid-template-columns={cols}
			>
