<script lang="ts">
	...
	const isNew = $derived(data.guide === null);
	...
	const overLimit = $derived(body.length > data.bodyMax);
	...
	<section class="rounded-2xl border border-border bg-bg-elev p-5">
		...
		<p class="mt-2 text-[12px] {overLimit ? 'text-[#ef7a6d]' : 'text-text-4'}">
			{m.mcp_guide_body_chars({
				...
				max: data.bodyMax.toLocaleString(getLocale())
			})}
		</p>
	</section>
