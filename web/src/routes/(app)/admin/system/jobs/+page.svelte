...
<div class="mb-6 flex items-end gap-4">
	<div>
		...
		<p class="mt-1 max-w-xl text-[14px] text-text-3">{m.jobs_description()}</p>
	</div>
	<div class="ml-auto">
...
<div class="mb-6 rounded-2xl border border-border bg-bg-elev p-5">
	<div class="flex items-center justify-between gap-4">
		<div>
			<h2 class="text-[15px] font-semibold text-text">{m.jobs_enqueue_heading()}</h2>
			<p class="mt-1 text-[14px] text-text-3">{m.jobs_enqueue_description()}</p>
		</div>
		<form
			method="post"
			action="?/sendTest"
	...
	{#if form?.message}
		<p class="mt-3 text-[14px] text-red-400">{form.message}</p>
	{/if}
</div>
		...
		{#each data.jobs as j (j.id)}
			...
			>
				<div class="min-w-0">
					...
					{#if j.priority !== 0}
						<span
							class="ml-2 rounded bg-surface px-1.5 py-0.5 text-[12px] text-text-3"
							title={m.jobs_priority_title()}
						>
							P{j.priority}
						</span>
					{/if}
					<span class="block truncate font-mono text-[12px] text-text-4">{j.id}</span>
					{#if j.lastError}
						<span class="mt-0.5 block max-w-md truncate text-[13px] text-red-400"
							>{j.lastError}</span
						>
					{/if}
				</div>
				<div>
					<span
						class="inline-flex rounded-full px-2.5 py-1 text-[12px] font-medium {statusStyles[
							j.status
						] ?? statusStyles.queued}"
					>
