<script lang="ts">
	...
	const assignee = $derived(ticket?.assignedAgentId ? resolveUser(ticket.assignedAgentId) : undefined);
...
<Drawer {open} {onclose} width={460}>
	{#if ticket}
		...
		<!-- Header -->
		<div class="flex items-center gap-2 border-b border-border px-4 py-3 text-[11px] text-text-3">
			...
			<span class="inline-flex items-center gap-1.5">
				...
				<span class="max-w-[160px] truncate">{ticket.orgName}</span>
			</span>
			<div class="ml-auto flex items-center gap-1">
				{#if pending}
					<span class="text-text-4"><Icon name="refresh" size={13} /></span>
				{/if}
				<button
					type="button"
					onclick={copyLink}
		...
		<div class="min-h-0 flex-1 overflow-y-auto px-4 py-4">
			...
			{#if canEdit}
				<input
					...
					class="mb-4 block w-full rounded-lg border border-transparent bg-transparent px-1 py-0.5 text-[18px] leading-tight font-semibold tracking-[-0.01em] text-text outline-none hover:border-border focus:border-border-strong focus:bg-surface"
				/>
			{:else}
				<h2 class="mb-4 px-1 text-[18px] leading-tight font-semibold tracking-[-0.01em] text-text">
					{ticket.subject}
				</h2>
			{/if}
			...
			<div class="space-y-1">
				...
				<div class="flex items-center gap-3">
					<span class="w-[84px] shrink-0 text-[12px] text-text-3">{m.tickets_field_status()}</span>
					<div class="relative">
						<button
							type="button"
							disabled={!canEdit}
							...
							>
								{#each TICKET_STATUSES as s (s.id)}
									...
									>
										...
										<span class="ml-auto text-accent {ticket.status === s.id ? 'opacity-100' : 'opacity-0'}">
				...
				<!-- Priority -->
				<div class="flex items-center gap-3">
					<span class="w-[84px] shrink-0 text-[12px] text-text-3">{m.tickets_field_priority()}</span>
					<div class="relative">
						<button
							type="button"
							disabled={!canEdit}
							...
							>
								{#each TICKET_PRIORITIES as p (p.id)}
									...
									>
										...
										<span class="ml-auto text-accent {ticket.priority === p.id ? 'opacity-100' : 'opacity-0'}">
											<Icon name="check" size={13} />
										</span>
									</button>
								{/each}
				...
				<!-- Category -->
				<div class="flex items-center gap-3">
					<span class="w-[84px] shrink-0 text-[12px] text-text-3">{m.tickets_field_category()}</span>
					<div class="relative">
						<button
							type="button"
							disabled={!canEdit}
						...
						>
							<span class="h-2 w-2 rounded-full" style:background={categoryMeta?.color ?? '#7c7c84'}></span>
							...
							>
								{#each TICKET_CATEGORIES as c (c.id)}
									...
									>
										...
										<span class="ml-auto text-accent {ticket.category === c.id ? 'opacity-100' : 'opacity-0'}">
											<Icon name="check" size={13} />
										</span>
									</button>
								{/each}
				...
				<!-- Assignee -->
				<div class="flex items-center gap-3">
					<span class="w-[84px] shrink-0 text-[12px] text-text-3">{m.tickets_field_assignee()}</span>
					<div class="relative">
						<button
							type="button"
							disabled={!canEdit}
						...
						{#if pop === 'assignee'}
							...
							>
								<button
									...
									class="flex w-full items-center gap-2.5 rounded-md px-2 py-1.5 text-left text-text-2 hover:bg-surface-2 hover:text-text disabled:opacity-50"
								>
									<span class="h-[18px] w-[18px] rounded-full border border-dashed border-border-strong"></span>
									<span class="text-[13px]">{m.common_unassigned()}</span>
									<span class="ml-auto text-accent {ticket.assignedAgentId == null ? 'opacity-100' : 'opacity-0'}">
										<Icon name="check" size={13} />
									</span>
								</button>
								{#each users as u (u.id)}
									...
									>
										...
										<span class="ml-auto text-accent {ticket.assignedAgentId === u.id ? 'opacity-100' : 'opacity-0'}">
											<Icon name="check" size={13} />
										</span>
									</button>
								{/each}
				...
				<div class="flex items-start gap-3 pt-0.5">
					<span class="w-[84px] shrink-0 pt-1.5 text-[12px] text-text-3">{m.tasks_tags()}</span>
					<div class="relative flex flex-wrap items-center gap-1.5">
						{#each ticket.tags as tag (tag)}
							{@const l = labelMeta(tag)}
							<span
								class="inline-flex items-center gap-1.5 rounded-md border border-border bg-surface px-2 py-1 text-[12px] text-text-2"
							>
						...
						{#if canEdit}
							<button
								...
								class="inline-flex items-center gap-1 rounded-md border border-dashed border-border px-2 py-1 text-[12px] text-text-3 transition-colors hover:border-border-strong hover:text-text"
							>
						...
						{:else if ticket.tags.length === 0}
							<span class="py-1 text-[12px] text-text-4">—</span>
						{/if}
					</div>
				</div>
			</div>
			...
			{#if ticket.description}
				<div class="mt-5 border-t border-border pt-4">
					...
					<p class="text-[13px] leading-relaxed whitespace-pre-wrap text-text-2">
						{ticket.description}
					</p>
				</div>
			{/if}
					...
					<ul class="flex flex-col gap-1">
						{#each linkedTasks as lt (lt.id)}
							<li>
								...
								>
									<StatusDot status={lt.status as StatusId} size={13} />
									<span class="font-mono text-[11px] text-text-4">{lt.displayId}</span>
									<span class="truncate text-[13px] text-text-2 group-hover:text-text">{lt.title}</span>
								</a>
							</li>
						{/each}
					</ul>
			...
			<!-- Metadata -->
			<div class="mt-5 grid grid-cols-2 gap-x-4 gap-y-2.5 border-t border-border pt-4 text-[12px]">
				<div>
					<div class="text-text-4">{m.tickets_meta_reporter()}</div>
					<div class="mt-0.5 flex items-center gap-1.5 text-text-2">
						{#if customer}<Avatar user={customer} size={16} />{/if}
		...
		<div class="border-t border-border px-4 py-3">
			<a
				...
				class="inline-flex w-full items-center justify-center gap-1.5 rounded-lg border border-border bg-surface py-2 text-[13px] text-text-2 transition-colors hover:border-border-strong hover:text-text"
			>
