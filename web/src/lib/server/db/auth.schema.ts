...
// ─── better-auth `mcp` plugin (OAuth 2.1 provider for MCP clients) ─────────
// Export names must match the plugin's model names exactly
// (oauthApplication / oauthAccessToken / oauthConsent). Column nullability
// mirrors `better-auth generate` output. `oauthConsent` is unused in practice
// (consent is skipped unless a client sends prompt=consent) but the plugin
// declares it, so the adapter needs the table.
